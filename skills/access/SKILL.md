...
7. `mkdir -p ~/.claude/channels/matrix/approved` then write
   ...
   and `@` with `__`). The channel server polls this dir and sends "you're in".
...
2. Add `<senderId>` to `allowFrom` (dedupe). Matrix user IDs look like
   `@user:server.com`.
3. Write back.
...
- Sender IDs are Matrix user IDs like `@user:server.com`. Don't validate
  format beyond checking they start with `@`.
- Room IDs look like `!abcdef:server.com`.
